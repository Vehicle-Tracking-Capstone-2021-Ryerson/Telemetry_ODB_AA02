...
{
  unsigned char buf[8];
  int __rpm = 0, __speed = 0, __throttle = 0, __airTemp = 0, __fuel_level = 0;
  ...
  if (buf[0] != 0x0)
  {
    extractAmbientTemp(buf, &__airTemp);
  }
  ...
  if (buf[0] != 0x0)
  {
    extractThrottlePos(buf, &__fuel_level);
  }

  SERIAL_PORT_MONITOR.print("FUEL LEVEL: ");
  SERIAL_PORT_MONITOR.println(__fuel_level);

  SERIAL_PORT_MONITOR.println(String(__rpm) + "," + String(__speed) + "," + String(__throttle) + "," + String(__airTemp) + "," + String(__fuel_level));
}
...
bool extractAmbientTemp(unsigned char *buf, int *temp)
{
  if (buf[1] == 0x41)
  {
    *temp = buf[3] - 40;
